...
export default function CargoCard({ cargo, onDelete, canDelete }) {
  ...
  const handleDelete = () => {
    const cargoId = cargo.id || cargo.idCargo;
    if (!cargoId) return;
    confirmDialog({
      ...
      <div className="p-6 md:p-8 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 gap-y-3 text-gray-700 text-base">
          <p className="flex items-center gap-2">
            ...
            <span className="font-medium">ID:</span> {cargo.idCargo || cargo.id}
          </p>
          <p className="flex items-center gap-2">
        ...
        <div className="flex flex-wrap gap-3 pt-6 border-t border-gray-100 mt-auto opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <Button
            ...
            onClick={() => router.push(`/dashboard/editar-cargo/${cargo.id || cargo.idCargo}`)}
          />
          <Button
            label="Ver"
            icon="pi pi-eye"
            className="p-button-sm p-button-outlined p-button-info flex-grow"
            onClick={() => router.push(`/dashboard/ver-cargo/${cargo.id || cargo.idCargo}`)}
          />
          {canDelete && (
            <Button
              label="Eliminar"
